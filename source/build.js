...
async function validOptions(options) {
	...
	if (await fs.exists(srcdir)) {
		...
		if (!stat.isDirectory()) 
			throw new Error(`srcdir must be directory: ${srcdir}`);
	} else {
	...
	if (await fs.exists(distdir)) {
		...
		if (!stat.isDirectory())
			throw new Error(`srcdir must be directory: ${distdir}`);
	}
...
async function build(file, options) {
	...
	if (/\.js$/.test(extname)) return await buildJs(file, options);
	if (/\.s(a|c)ss$/.test(extname)) return await buildSass(file, options);
	...
	await new Promise((resolve, reject)=> 
		...
		.on('data', (item)=> {
			if (item.stats.isFile())
				task = task.then(()=> build(item.path, options));
		})
	);
}
...
async function watch(options) {
	...
	function srcfile(file) {
		return path.relative(options.srcdir, file);
	}

	function distfile(file) {
		return path.resolve(options.distdir, srcfile(file));
	}
