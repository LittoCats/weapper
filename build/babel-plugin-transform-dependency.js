...
function getmid(name, file) {
  ...
  const mid = _shorthash.default.unique(file);
...
function buildDependency(mid, file) {
  ...
  const distFile = _path.default.resolve(options.distdir, 'miniprogram_npm', mid.path); // 如果目标文件存在，不需要重新生成


  if (_fsExtra.default.existsSync(distFile)) return;
  const babelConfig = {
    plugins: [plugin],
    // node_modules 依赖，不再次进行 babel 转码
    // 这就要求 node_modules 依赖，已经对兼容性进行了处理
    configFile: _path.default.resolve(__dirname, 'babel.config.js')
  };
...
function walkDependency(origin, node) {
  ...
  if (/^\./.test(node.value)) {
    ...
    const __dirname = _path.default.dirname(origin); // current 是依赖相当前模块目录的相对跳径


    const file = require.resolve(_path.default.resolve(__dirname, node.value));

    const current = _path.default.relative(__dirname, file);
    ...
    // node_modules 中的模块
    const __dirname = _path.default.resolve(process.cwd(), 'node_modules'); // current 是依赖模块相对 node_modules 目录的相对路径
...
function travelProcessEnvNODE_ENV(path, {
  opts
}) {
  const options = utils.buildOptions;
...
function travelRequire(path, stat) {
  ...
  if (!t.isStringLiteral(name)) throw new Error(`Argument of require must be StringLiteral.`);
  walkDependency(path.hub.file.opts.filename, name);
  return true;
