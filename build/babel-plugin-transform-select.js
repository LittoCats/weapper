...
function travelSelect(path, {
  ...
  const properties = config.properties.filter(node => node.key.name == env); // production 环境中，如果没有配制 production，抛出错误

  if (properties.length == 0) {
    if (env == 'production') throw new Error(`No config found for env: ${env.red}`);
    console.log(`No config found for env: ${env.yellow}, 
			use config for env: ${'development'.yellow}`);
    const property = config.properties.filter(node => node.key.name == 'development').pop();
    if (property) properties.push(property);
  }

  if (properties.length == 0) throw new Error(`No config found for env: ${env.red}`);
